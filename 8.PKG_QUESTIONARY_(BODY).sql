...
BEGIN
	UPDATE QUESTIONARY_ANSWER_TYPE
		SET ANSWER_TYPE = P_vANSWER_TYPE
		WHERE ID = P_nID;
...
-- Create Questionary with returning ID
FUNCTION fCREATE_Q_QUESTIONARY(P_nTEMPLATE_ID	IN NUMBER,
								P_dDATE_BEG		IN DATE,
								P_dDATE_END		IN DATE DEFAULT NULL) RETURN NUMBER
AS
...
BEGIN
	INSERT INTO QUESTIONARY
		...
		VALUES (P_nTEMPLATE_ID,
				P_dDATE_BEG,
				P_dDATE_END) RETURNING ID INTO QUESTIONARY_ID;
	RETURN QUESTIONARY_ID;
END fCREATE_Q_QUESTIONARY;
...
AS
	QUESTIONARY_ID NUMBER;
BEGIN
	QUESTIONARY_ID := fCREATE_Q_QUESTIONARY(P_nTEMPLATE_ID	=>P_nTEMPLATE_ID,
											P_dDATE_BEG		=>P_dDATE_BEG,
											P_dDATE_END		=>P_dDATE_END);
...
-- Create Questionary User with returning ID
FUNCTION fCREATE_Q_USER(P_nUSER_ID			IN NUMBER,
						P_nQUESTIONARY_ID	IN NUMBER) RETURN NUMBER
AS
	USER_ID NUMBER;
BEGIN
	INSERT INTO QUESTIONARY_USERS
				...
				QUESTIONARY_ID)
		VALUES (P_nUSER_ID,
				P_nQUESTIONARY_ID) RETURNING ID INTO USER_ID;
...
AS
	USER_ID NUMBER;
BEGIN
	USER_ID := fCREATE_Q_USER(P_nUSER_ID			=>P_nUSER_ID,
								P_nQUESTIONARY_ID	=>P_nQUESTIONARY_ID);
END pCREATE_Q_USER;
...
BEGIN
	...
	IF ((P_SANSWER IS NOT NULL AND P_nANSWER IS NULL AND P_DANSWER IS NULL AND P_BANSWER IS NULL AND P_CANSWER IS NULL)
		OR (P_SANSWER IS NULL AND P_nANSWER IS NOT NULL AND P_DANSWER IS NULL AND P_BANSWER IS NULL AND P_CANSWER IS NULL)
		OR (P_SANSWER IS NULL AND P_nANSWER IS NULL AND P_DANSWER IS NOT NULL AND P_BANSWER IS NULL AND P_CANSWER IS NULL)
		OR (P_SANSWER IS NULL AND P_nANSWER IS NULL AND P_DANSWER IS NULL AND P_BANSWER IS NOT NULL AND P_CANSWER IS NULL)
		OR (P_SANSWER IS NULL AND P_nANSWER IS NULL AND P_DANSWER IS NULL AND P_BANSWER IS NULL AND P_CANSWER IS NOT NULL))
	THEN
...
BEGIN
	...
	nQUESTIONARY_ID:=fCREATE_Q_QUESTIONARY(P_nTEMPLATE_ID 	=>P_nTEMPLATE_ID,
										P_dDATE_BEG		 	=>P_dDATE_BEG,
										P_dDATE_END		 	=>P_dDATE_END);
				
	nQ_USER_ID:=fCREATE_Q_USER(P_nUSER_ID			=>P_nUSER_ID,
								P_nQUESTIONARY_ID	=>nQUESTIONARY_ID);
